...
- [ ] Resolution-affecting CLI flag added (offline / remote / config / index / similar) → forwarded in the project's subprocess-spawn helper (the single env-composition function) AND documented in the env-var reference. Presentation flag added (log-level / format / color) → never forwarded via env
