...
| **high** | One-Way Door Medium; new subcommand; new index format; new storage layout; 1–2 subsystems; label `enhancement`, `feature` | `new grimoire env composition command`, `add referrers API`, `introduce tag lock cache` |
...
1. `/swarm-plan "add --json flag to grimoire ls"` → tier=**low**, no overlays, confident.
