...
| Marker | Tier impact |
|---|---|
| New top-level directory under `src/` | → **max** |
| `.github/workflows/**` changes | Adds `--breadth=full` minimum; security review required |
| Network / registry I/O paths | Adds `--breadth=full`; `--codex` auto-on at high (security-sensitive) |
| Core data-flow modules under `src/` | Adds `--breadth=adversarial` at high+ |
...
| `--doc-reviewer=haiku` | Diff touches ≤2 doc files (`docs/**/*.md` or `CHANGELOG.md`) AND does not touch the primary user guide under `docs/` |
...
3. `/swarm-review 143` where PR #143 has labels
   `breaking-change` + `enhancement`, touches `src/`
...
4. `/swarm-review --base=v0.5.0` on branch 30 commits ahead
   ...
   gate).
5. `/swarm-review` with 4 files changed across `src/` and
   `src/` → metrics say `low` (size) but `high` (≥2
