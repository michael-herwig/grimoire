...
- Run: `cargo nextest run <test_name>` or `cargo test -- <test_name> --nocapture`
- Use `tempfile::tempdir()` for isolated filesystem tests
- Test error-enum variants explicitly

### Pytest Acceptance Tests

- Location: `test/tests/test_*.py`
- Key fixtures: `grimoire` (GrimRunner), `published_package`, `published_two_versions`, `unique_repo`
- Runner API: `grimoire.json("command", pkg.short)`, `grimoire.plain(...)`, `grimoire.run(..., check=False)`
- Assertions: `assert_symlink_exists()`, `assert_not_exists()`, `assert_dir_exists()`
- Custom packages: `make_package(grimoire, repo, tag, tmp_path, bins=[...], env=[...])`
