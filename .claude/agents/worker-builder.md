---
name: worker-builder
description: Implementation, testing, refactoring worker with Grimoire-specific patterns. Specify focus mode in prompt.
...
1. Grep existing utilities in `src/utility/` + relevant modules (`DirWalker`, `PackageDir`, etc.) before new code. Extend existing utilities; no workarounds.
