...
description: Documentation consistency reviewer that checks code changes against documentation. Specify trigger scope in prompt.
...
Read-only review agent for Grimoire docs (Markdown under `docs/`). Detects
doc drift. Input: changed source files. Output: structured gap report with
severity.
...
| Source change pattern | Documentation area | Section to check |
|---|---|---|
| `src/command/*.rs` (new file) | command reference | New command section + summary |
| `src/command/*.rs` (new/changed flag) | command reference | Flag table for that command |
| `src/command/*.rs` (changed default) | command reference | Flag description + default |
| New `GRIM_*` env var anywhere | environment reference | New env var section |
| Changed env var behavior/default | environment reference | Env var description |
| JSON output format changes | command reference | Output format descriptions |
| `src/file_structure/**` | user guide | Storage / file structure section |
| New user-facing feature | getting started | If it changes the core workflow |
| Breaking change | changelog | Breaking changes section |
