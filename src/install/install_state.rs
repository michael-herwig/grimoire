...
pub struct ClientRecord {
    ...
    /// For a rule this is the index `<name>.md` file beside any
    /// [`Self::support_dir`].
    pub target: PathBuf,
    ...
    /// For a multi-file rule, the sibling support directory installed beside
    /// the index `target` (`<parent>/<name>/`). Absent for skills and
    /// single-file rules; old records without the field load as `None`.
...
impl ClientRecord {
    ...
    /// (the target plus, for a multi-file rule, its sibling support dir).
    /// The integrity gate compares this against [`Self::content_hash`].
    ///
    /// # Errors
    ///
    /// Any I/O error from walking or reading the footprint.
    pub fn current_hash(&self) -> std::io::Result<Digest> {
        crate::install::content_hash::footprint_hash(&self.target, self.support_dir.as_deref())
    }
}
